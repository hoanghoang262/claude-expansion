...
- **Systemic** (3+ failures, same pattern) → STOP. This is an architectural problem.
  - Write concern: `docs/concerns/CONCERN-{id}.md` — use `templates/concern-template.md`
...
- Write to `docs/learnings/{date}-{pattern}.md` using `templates/learning-template.md`.
  - Does this indicate a `.claude/rules/` file needs updating?
  - If yes → flag for rules update (do NOT update mid-session, wait until verify complete)
